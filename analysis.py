...
def main():
        ...
        for j in range(1):
            ...
            ate.append(euclid_distance_est)
            ate_dr.append(euclid_distance_dr)


        # plt.savefig("Simulation Plot Big Circular")
        # print(euclid_distance_dr)
        # print(euclid_distance_est)
        plt.hist(ate, bins=20)
        plt.hist(ate_dr, bins = 20, color="r")
        plt.xlabel('ATE (m)')
        plt.ylabel('Frequency')
        plt.title('ATE for Pre-planned Trajectory 3')
        plt.savefig('hist 4.png')
        plt.show()
        
        df = pd.DataFrame(ate)
        df2 = pd.DataFrame(ate_dr)
        df2.to_csv('Trajectory 1 DR.csv')
        df.to_csv('Trajectory 1.csv')
        average = sum(ate) / len(ate)
        average2 = sum(ate_dr)/len(ate_dr)
